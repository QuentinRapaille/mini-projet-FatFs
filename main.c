 ...
 ******************************************************************************
 * @file           : main.c
 * @brief          : Album photo sur carte SD et lecture de fichier wav
 *
 ******************************************************************************
 * @attention
 *
...
void Affichage_Pic(void const * argument)
{
  /* USER CODE BEGIN Affichage_Pic */
	...
	if (FATFS_LinkDriver(&SD_Driver, SDPath) == 0) {
		...
		ubNumberOfFiles = Storage_GetDirectoryWAVFiles("/Son",
				pDirectoryFiles);
	}else {
	...
	for (counter=0;counter<ubNumberOfFiles;counter++){
		sprintf(str, "Son/%s",pDirectoryFiles[counter]);
		BSP_LCD_DisplayStringAtLine(counter, str);
	}

	FIL F1;
	sprintf(str, "Son/%s",pDirectoryFiles[0]);
	if (f_open(&F1, (TCHAR const*)str, FA_READ) != FR_OK)
	  {
	    while(1)
	    {
	    }
	 ...
	 uint32_t BytesRead = 0;
	  if (f_read ( &F1, sector, 44, (UINT *)&BytesRead) != FR_OK)
	  {
		  void * vector;
	    while(1)
	    {
	  ...
	  uint32_t FileAddress = (uint32_t)sector;

	  /* Read bitmap size */
	  uint32_t size = *(uint32_t *) (FileAddress + 24);
